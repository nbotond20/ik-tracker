...
const DashBoardPage: NextPage = () => {
  ...
  const [semester, setSemester] = useState(session?.user?.currentSemester ?? 1)

  const { data: subjectProgresses, refetch: refetchSubjectProgresses } = api.subjectProgress.getBySemester.useQuery(
    {
      semester: semester,
    },
        ...
        <div className="flex items-baseline justify-between border-b border-gray-200 pt-12 pb-6">
          ...
          <div className="flex items-center">
            ...
            >
              ...
              {Array.from({ length: session?.user?.currentSemester || 0 }, (_, i) => i + 1).map(idx => (
                <option key={idx} value={semester} selected={idx === semester}>
                  Semester {idx}
                </option>
              ))}
            </select>
